...
public sealed class ManifestScannerTests
{
    ...
    public async Task Scans_nested_files_and_preserves_relative_paths()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(temp.Path, "a.txt"), "abc", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(Path.Combine(temp.Path, "nested", "b.txt"), "12345", TestContext.Current.CancellationToken);
        var writer = new RecordingWriter();
        var result = await Scanner().ScanAsync(Request(temp.Path), writer, cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        using var temp = new TemporaryDirectory(); await File.WriteAllTextAsync(Path.Combine(temp.Path, "cloud.txt"), "bytes", TestContext.Current.CancellationToken);
        var writer = new RecordingWriter();
    ...
    public async Task Online_only_cloud_bytes_are_skipped_and_reported()
    {
        using var temp = new TemporaryDirectory(); await File.WriteAllTextAsync(Path.Combine(temp.Path, "cloud.txt"), "bytes", TestContext.Current.CancellationToken);
        var writer = new RecordingWriter();
        await new ManifestScanner(new FakeCloud(CloudContentState.OnlineOnly)).ScanAsync(Request(temp.Path), writer, cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        using var temp = new TemporaryDirectory(); using var customCancellation = new CancellationTokenSource(); using var cancellation = CancellationTokenSource.CreateLinkedTokenSource(TestContext.Current.CancellationToken, customCancellation.Token); customCancellation.Cancel();
    ...
    public async Task Reparse_directories_are_not_followed()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outside.Path, "outside.txt"), "secret", TestContext.Current.CancellationToken);
        try { Directory.CreateSymbolicLink(Path.Combine(root.Path, "link"), outside.Path); }
        catch (UnauthorizedAccessException) { return; }
        var writer = new RecordingWriter(); await Scanner().ScanAsync(Request(root.Path), writer, cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        using var temp = new TemporaryDirectory(); await File.WriteAllTextAsync(Path.Combine(temp.Path, "outlook.ost"), "cache", TestContext.Current.CancellationToken);
        var result = await Scanner().ScanAsync(Request(temp.Path), new RecordingWriter(), cancellationToken: TestContext.Current.CancellationToken);
