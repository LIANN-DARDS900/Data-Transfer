...
public sealed class ApprovedNetworkShareDetectorTests
{
    ...
    [Fact] public async Task Cancellation_is_observed() { using var customCancellation = new CancellationTokenSource(); using var cancellation = CancellationTokenSource.CreateLinkedTokenSource(TestContext.Current.CancellationToken, customCancellation.Token); customCancellation.Cancel(); var policy = new PolicyProfile(1, true, false, false, false, VerificationLevel.Strong, ConflictPolicy.KeepBoth, [@"\\fileserver\migration"]); await Assert.ThrowsAnyAsync<OperationCanceledException>(() => new ApprovedNetworkShareDetector().DetectAsync(policy, cancellation.Token)); }
}
