...
public sealed class DestinationValidatorTests
{
    ...
    public async Task Rejects_overlapping_paths(string scenario)
    {
        ...
        var result = await Validator().ValidateAsync(Context(destination, [source]), TestContext.Current.CancellationToken); Assert.False(result.IsValid); Assert.Contains(result.Errors, e => e.Category == ErrorCategory.InvalidPath);
    }
    [Fact]
    public async Task Rejects_application_install_path() { var result = await Validator().ValidateAsync(Context(AppContext.BaseDirectory, [Path.GetTempPath()]), TestContext.Current.CancellationToken); Assert.Contains(result.Errors, e => e.Category == ErrorCategory.PolicyForbidden); }
    [Theory]
    [MemberData(nameof(ProtectedPaths))]
    public async Task Rejects_windows_and_program_files_paths(string protectedPath) { var result = await Validator().ValidateAsync(Context(protectedPath, [Path.Combine(Path.GetTempPath(), "source-not-protected")]), TestContext.Current.CancellationToken); Assert.Contains(result.Errors, e => e.Category == ErrorCategory.PolicyForbidden); }
    ...
    [Fact]
    public async Task Rejects_disconnected_external_media() { using var temp = new Temp(); var result = await Validator().ValidateAsync(Context(temp.Path, [], volume: null), TestContext.Current.CancellationToken); Assert.Contains(result.Errors, e => e.Category == ErrorCategory.StorageDisconnected); }
    [Theory]
    ...
    public async Task Approved_unc_is_allowed_and_unapproved_unc_is_rejected_without_network_access()
    {
        ...
        Assert.True((await Validator().ValidateAsync(new(approvedPlan, [@"C:\source"], policy, null), TestContext.Current.CancellationToken)).IsValid);
        var denied = await Validator().ValidateAsync(new(approvedPlan with { DestinationPath = @"\\server\other" }, [@"C:\source"], policy, null), TestContext.Current.CancellationToken); Assert.Contains(denied.Errors, e => e.Category == ErrorCategory.PolicyForbidden);
    }
    [Fact]
    public async Task Write_probe_cleanup_is_delegated_and_no_artifact_remains() { using var temp = new Temp(); var probe = new RecordingProbe(); var result = await new DestinationValidator(probe).ValidateAsync(Context(temp.Path, [], Volume(temp.Path, true, "disk", "NTFS", 100)), TestContext.Current.CancellationToken); Assert.True(result.IsValid); Assert.Equal(temp.Path, probe.Path); Assert.Empty(Directory.EnumerateFiles(temp.Path, ".robotransfer-write-*")); }
