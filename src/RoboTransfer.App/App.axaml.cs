...
public sealed partial class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        ...
        var data = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "RoboTransfer"); collection.AddSingleton<IPolicyProvider>(_ => new JsonPolicyProvider(Path.Combine(data, "policy.json"))); collection.AddSingleton<IMigrationJournal>(provider => new JsonMigrationJournal(Path.Combine(data, "sessions"), provider.GetRequiredService<ILogger<JsonMigrationJournal>>()));
