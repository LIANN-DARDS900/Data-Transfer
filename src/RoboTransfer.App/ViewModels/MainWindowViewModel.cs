...
public sealed class MainWindowViewModel : ViewModelBase
{
    ...
    public MainWindowViewModel(ICapabilityDetector detector, IMigrationPlanner planner, IPolicyProvider policies)
    {
        this.detector = detector; this.planner = planner; this.policies = policies; RefreshCommand = new AsyncCommand(RefreshAsync, ShowError);
        Workflow = [new("1", "Environment", true, true), new("2", "Source", false, true), new("3", "Migration plan", false, true), new("4", "Transfer", false, false), new("5", "Verification", false, false), new("6", "Report", false, false)];
    }
    public IReadOnlyList<WorkflowStep> Workflow { get; } public IReadOnlyList<MigrationRole> Roles { get; } = Enum.GetValues<MigrationRole>();
    ...
    public MigrationRole Role { get => role; set => Set(ref role, value); } public UserProfile? SelectedProfile { get => selectedProfile; set { if (Set(ref selectedProfile, value)) OnPropertyChanged(nameof(ProfileDetail)); } }
    ...
    private async Task RefreshAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var capabilities = await detector.DetectAsync(policy, cancellationToken); var plan = planner.CreatePlan(policy, capabilities);
